...
[t_100days_global1, sol_100days_global1] = euler(@(t,y) model1(t,y), [0, 24*100], [800,600,20], 0.01);
[t_100days_global2, sol_100days_global2] = euler(@(t,y) model2(t,y), [0, 24*100], [800,600,20], 0.01);
...
pdata_36hours1_global2 = sol_100days_global2(233600:237600, 3);
...
wrmse1_global3 = calculate_wrmse(norm_plasma_global11, exp_plasma1);

plasma_36hr_model_global2 = pdata_36hours1_global2(time_indices);
norm_plasma_global12 = plasma_36hr_model_global2(selected_indices1);
wrmse1_global4 = calculate_wrmse(norm_plasma_global12, exp_plasma1);
...
plot(x, cdata_36hours1_global2, 'LineWidth', 2.0, 'Color', colormap_jet(2,:), 'DisplayName', sprintf('Ind Fit (NRMSE: %.3f)', wrmse1_global2));
...
scatter(time_exp1, model_points_global2, 80, colormap_jet(2,:), 'filled', 'MarkerEdgeColor', 'k', 'DisplayName', 'Global Fit Points');
...
plot(x, pdata_36hours1_global2, 'LineWidth', 2.0, 'Color', colormap_jet(2,:), 'DisplayName', sprintf('Global Fit (NRMSE: %.3f)', wrmse1_global4));
model_points_global3 = interp1(x, pdata_36hours1_global1, time_exp1);
model_points_global4 = interp1(x, pdata_36hours1_global2, time_exp1);
errorbar(time_exp1, plasma_conc_exp1, plasma_std, 'k--', 'LineWidth', 2.0, 'DisplayName', 'Blattner2020 - CSF', 'CapSize', 6, 'MarkerFaceColor', 'k', 'MarkerEdgeColor', 'k', 'Marker', 'o');
...
scatter(time_exp1, model_points_global3, 80, colormap_jet(1,:), 'filled', 'MarkerEdgeColor', 'k', 'DisplayName', 'Global Fit Points');
scatter(time_exp1, model_points_global4, 80, colormap_jet(2,:), 'filled', 'MarkerEdgeColor', 'k', 'DisplayName', 'Global Fit Points');
xline(1600, 'k--', 'LineWidth', 1.0, 'Alpha', 0.3, 'HandleVisibility', 'off');
xline(2400, 'k--', 'LineWidth', 1.0, 'Alpha', 0.3, 'HandleVisibility', 'off');
xlim([2336, 2386]);
xticks(0:200:4000);
...
model_csf_liu = interp1(x, cdata_36hours1_global2, time_exp1_adjusted);
model_plasma_liu = interp1(x, pdata_36hours1_global2, time_exp1_adjusted);
...
% Plot
figure();
stem(time_hours_plot, residuals_liu_csf, 'filled', ...
    'Color', [0.2 0.7 0.3], 'LineWidth', 1.5)
hold on
yline(0, 'k--', 'LineWidth', 1.5)
xlabel('Time (hr)', 'FontWeight', 'bold')
ylabel('Residual (pg/ml)', 'FontWeight', 'bold')
title('Liu et al. 2023 - CSF', 'FontWeight', 'bold')
text(1, max(abs(residuals_liu_csf))*0.85, ...
    sprintf('Mean = %.2f pg/ml', mean(residuals_liu_csf)), 'FontSize', 9)
grid on
hold off

figure();
stem(time_hours_plot, residuals_liu_plasma, 'filled', ...
    'Color', [0.8 0.5 0.1], 'LineWidth', 1.5)
hold on
yline(0, 'k--', 'LineWidth', 1.5)
xlabel('Time (hr)', 'FontWeight', 'bold')
ylabel('Residual (pg/ml)', 'FontWeight', 'bold')
title('Liu et al. 2023 - Plasma', 'FontWeight', 'bold')
text(1, max(abs(residuals_liu_plasma))*0.85, ...
    sprintf('Mean = %.2f pg/ml', mean(residuals_liu_plasma)), 'FontSize', 9)
grid on
hold off
