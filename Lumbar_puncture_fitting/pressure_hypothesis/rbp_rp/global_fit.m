...
function dydt_n = model(t, y, params)
    ...
    r_bp = 1.319956380713120;
    r_p = 0.005459771155221;
    ...
    dydt_n = zeros(3, 1);
    dydt_n(1) = A * sw_cycle + sigma_A * A * (1 - sw_cycle) - (r_bp * sw_cycle + sigma_bp * r_bp * (1 - sw_cycle) + r_bp * sw_cycle + sigma_bc * r_bp * (1 - sw_cycle)) * y(1);
    dydt_n(2) = (r_bp * sw_cycle + sigma_bc * r_bp * (1 - sw_cycle)) * y(1) - (r_p * sw_cycle + sigma_cp * r_p * (1 - sw_cycle)) * y(2);
    dydt_n(3) = (r_bp * sw_cycle + sigma_bp * r_bp * (1 - sw_cycle)) * y(1) + (r_p * sw_cycle + sigma_cp * r_p * (1 - sw_cycle)) * y(2) - (r_p * sw_cycle + sigma_p * r_p * (1 - sw_cycle)) * y(3);
end
...
function [total_error] = objective_function_fmincon(params, exp_csf1, exp_csf2, exp_csf3, exp_plasma)
    ...
    errorr_bp1 = (plasma_model - exp_plasma) .^ 2;
    ...
    error_P1 = sqrt(sum(errorr_bp1) / length(exp_plasma));
    ...
    fprintf('Parameters: r_bp=%.5f, r_p=%.5f\n', ...
            params(1), params(2));
...
function plot_parameter_space(solutions, num_starts)
    ...
    params_matrix = zeros(num_starts, 2);
    ...
    saveas(gcf, 'global_fit_error1/parameter_space.png');
    print('global_fit_error1/parameter_space_highres', '-dpng', '-r300');

    % NEW CODE: Save parameter space data to CSV files
    for param_idx = 1:2
        % Create and open CSV file for this parameter
        param_name = param_names{param_idx};
        csv_filename = sprintf('global_fit_error1/parameter_space_%s.csv', param_name);
...
if ~exist('global_fit_error1', 'dir')
    mkdir('global_fit_error1');
end
...
for i = 1:num_starts
    ...
    save(sprintf('global_fit_error1/run_%d_results.mat', i), ...
        'run_results');
    
    % Log the results
    fprintf('\nRun %d:\n', i);
    fprintf('Parameters: r_bp=%.5f, r_p=%.5f\n', ...
        solutions(i).params(1), solutions(i).params(2));
...
save('global_fit_error1/final_results.mat', 'final_results');

% Create a text file to save results
fileID = fopen('global_fit_error1/detailed_results.txt', 'w');
...
for i = 1:num_starts
    ...
    fprintf(fileID, 'Parameters: r_bp=%.5f, r_p=%.5f\n', ...
        solutions(i).params(1), solutions(i).params(2));
    ...
    errorr_bp1 = (plasma_model1 - exp_plasma) .^ 2;
...
for i = 1:num_starts
    ...
    writematrix(loss_data, sprintf('global_fit_error1/loss_curve_run_%d.csv', i));
end
...
legend;
hold off;
saveas(gcf, 'global_fit_error1/loss_curves.png');
