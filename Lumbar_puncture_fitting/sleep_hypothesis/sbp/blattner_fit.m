...
function plot_parameter_space(solutions, num_starts)
    ...
    saveas(gcf, 'blattner_exp_fit_error1/parameter_space.png');
    print('blattner_exp_fit_error1/parameter_space_highres', '-dpng', '-r300');

    % NEW CODE: Save parameter space data to CSV files
    for param_idx = 1:1
        % Create and open CSV file for this parameter
        param_name = param_names{param_idx};
        csv_filename = sprintf('blattner_exp_fit_error1/parameter_space_%s.csv', param_name);
...
if ~exist('blattner_exp_fit_error1', 'dir')
    mkdir('blattner_exp_fit_error1');
end
...
for i = 1:num_starts
    ...
    save(sprintf('blattner_exp_fit_error1/run_%d_results.mat', i), ...
        'run_results');
...
save('blattner_exp_fit_error1/final_results.mat', 'final_results');

% Create a text file to save results
fileID = fopen('blattner_exp_fit_error1/detailed_results.txt', 'w');
...
for i = 1:num_starts
    ...
    writematrix(loss_data, sprintf('blattner_exp_fit_error1/loss_curve_run_%d.csv', i));
end
...
legend;
hold off;
saveas(gcf, 'blattner_exp_fit_error1/loss_curves.png');
