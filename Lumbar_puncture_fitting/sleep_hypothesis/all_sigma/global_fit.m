...
function [total_error] = objective_function_fmincon(params, exp_csf1, exp_csf2, exp_csf3, exp_plasma)
    ...
    fprintf('Parameters: s_bc=%.5f, s_cp=%.5f, s_p=%.5f, s_A=%.5f, s_bp=%.5f\n', ...
...
function plot_parameter_space(solutions, num_starts)
    ...
    param_names = {'s_bc', 's_cp', 's_p', 's_A', 's_bp'};
...
num_starts = 20;
initial_guess = [0.8, 2.5, 1.5, 0.8, 5];
lb = [0.01, 0.01, 0.01, 0.731, 0.1];
ub = [1.185, 4.173, 4.028, 0.9, 6.607];
...
for i = 1:num_starts
    ...
    fprintf(fileID, 'Parameters: s_bc=%.5f, s_cp=%.5f, s_p=%.5f, s_A=%.5f, s_bp=%.5f\n', ...
        solutions(i).params(1), solutions(i).params(2), solutions(i).params(3), solutions(i).params(4), solutions(i).params(5));
