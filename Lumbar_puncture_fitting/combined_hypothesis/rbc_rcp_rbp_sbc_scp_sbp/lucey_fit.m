...
function [total_error] = objective_function_fmincon(params, exp_csf1)
    ...
    fprintf('Parameters: r_bc=%.5f, r_bp=%.5f, r_cp=%.5f, s_bc=%.5f, s_bp=%.5f, s_cp=%.5f\n', ...
...
function plot_parameter_space(solutions, num_starts)
    ...
    param_names = {'r_bc', 'r_bp', 'r_cp', 's_bc', 's_bp', 's_cp'};
    ...
    saveas(gcf, 'lucey_exp_fit2/parameter_space.png');
    print('lucey_exp_fit2/parameter_space_highres', '-dpng', '-r300');

    % NEW CODE: Save parameter space data to CSV files
    for param_idx = 1:6
        % Create and open CSV file for this parameter
        param_name = param_names{param_idx};
        csv_filename = sprintf('lucey_exp_fit2/parameter_space_%s.csv', param_name);
...
num_starts = 20;
initial_guess = [3, 0.188, 0.004, 1.8, 3.429, 6];
lb = [2.432, 0.1, 0.0001, 0.01, 0.01, 0.01];
ub = [5, 5, 0.00532, 2.153, 7, 6.450];
...
if ~exist('lucey_exp_fit2', 'dir')
    mkdir('lucey_exp_fit2');
end
...
for i = 1:num_starts
    ...
    save(sprintf('lucey_exp_fit2/run_%d_results.mat', i), ...
        'run_results');
...
save('lucey_exp_fit2/final_results.mat', 'final_results');

% Create a text file to save results
fileID = fopen('lucey_exp_fit2/detailed_results.txt', 'w');
...
for i = 1:num_starts
    ...
    fprintf(fileID, 'Parameters: r_bc=%.5f, r_bp=%.5f, r_cp=%.5f, s_bc=%.5f, s_bp=%.5f, s_cp=%.5f\n', ...
        solutions(i).params(1), solutions(i).params(2), solutions(i).params(3), solutions(i).params(4), solutions(i).params(5), solutions(i).params(6));
...
for i = 1:num_starts
    ...
    writematrix(loss_data, sprintf('lucey_exp_fit2/loss_curve_run_%d.csv', i));
end
...
legend;
hold off;
saveas(gcf, 'lucey_exp_fit2/loss_curves.png');
