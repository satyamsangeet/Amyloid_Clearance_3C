...
num_starts = 20;
initial_guess = [2.5, 0.004, 0.242, 0.8, 2.5, 6.607];
lb = [1.319, 0.0001, 0.1, 0.1, 0.1, 0.1];
ub = [5, 0.00545, 5, 1.185, 4.173, 7];
...
for i = 1:num_starts
    ...
    errorsigma_bc1 = (plasma_model1 - exp_plasma) .^ 2;

    error_C1 = sqrt(sum(errors_C1) / length(exp_csf1));
    error_C2 = sqrt(sum(errors_C2) / length(exp_csf2));
    error_C3 = sqrt(sum(errors_C3) / length(exp_csf3));
    error_P1 = sqrt(sum(errorsigma_bc1) / length(exp_plasma));
