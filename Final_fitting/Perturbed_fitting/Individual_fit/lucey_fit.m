...
function plot_parameter_space(solutions, num_starts)
    ...
    saveas(gcf, 'lucey_exp_fit/parameter_space.png');
    print('lucey_exp_fit/parameter_space_highres', '-dpng', '-r300');

    for param_idx = 1:4
        param_name = param_names{param_idx};
        csv_filename = sprintf('lucey_exp_fit/parameter_space_%s.csv', param_name);
...
for i = 1:num_starts
    ...
    save(sprintf('lucey_exp_fit/run_%d_results.mat', i), 'run_results');
...
fileID_corr = fopen('lucey_exp_fit/correlation_matrix.txt', 'w');
...
saveas(gcf, 'lucey_exp_fit/correlation_heatmap.png');
...
save('lucey_exp_fit/final_results.mat', 'final_results');


fileID = fopen('lucey_exp_fit/detailed_results.txt', 'w');
...
for i = 1:num_starts
    ...
    writematrix(loss_data, sprintf('lucey_exp_fit/loss_curve_run_%d.csv', i));
end
...
legend;
hold off;
saveas(gcf, 'lucey_exp_fit/loss_curves.png');
