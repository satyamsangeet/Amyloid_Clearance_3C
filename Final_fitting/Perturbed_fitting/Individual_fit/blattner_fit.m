...
function plot_parameter_space(solutions, num_starts)
    ...
    saveas(gcf, 'blattner_exp_fit/parameter_space.png');
    print('blattner_exp_fit/parameter_space_highres', '-dpng', '-r300');

    for param_idx = 1:4
        param_name = param_names{param_idx};
        csv_filename = sprintf('blattner_exp_fit/parameter_space_%s.csv', param_name);
...
if ~exist('blattner_exp_fit', 'dir')
    mkdir('blattner_exp_fit');
end
...
for i = 1:num_starts
    ...
    save(sprintf('blattner_exp_fit/run_%d_results.mat', i), 'run_results');
...
fileID_corr = fopen('blattner_exp_fit/correlation_matrix.txt', 'w');
...
saveas(gcf, 'blattner_exp_fit/correlation_heatmap.png');
...
save('blattner_exp_fit/final_results.mat', 'final_results');

fileID = fopen('blattner_exp_fit/detailed_results.txt', 'w');
...
for i = 1:num_starts
    ...
    writematrix(loss_data, sprintf('blattner_exp_fit/loss_curve_run_%d.csv', i));
end
...
legend;
hold off;
saveas(gcf, 'blattner_exp_fit/loss_curves.png');
