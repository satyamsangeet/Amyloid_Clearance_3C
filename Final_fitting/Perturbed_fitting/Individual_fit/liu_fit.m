...
function [total_error] = objective_function_fmincon(params, exp_csf3, exp_plasma1)
    ...
    fprintf('Individual Study Errors: CSF3=%.4f, Plasma=%.4f\n', error_C3, error_P1);
...
function plot_parameter_space(solutions, num_starts)
    ...
    saveas(gcf, 'liu_exp_fit1/parameter_space.png');
    print('liu_exp_fit1/parameter_space_highres', '-dpng', '-r300');

    for param_idx = 1:4
        param_name = param_names{param_idx};
        csv_filename = sprintf('liu_exp_fit1/parameter_space_%s.csv', param_name);
...
for i = 1:num_starts
    ...
    save(sprintf('liu_exp_fit1/run_%d_results.mat', i), 'run_results');
...
fileID_corr = fopen('liu_exp_fit1/correlation_matrix.txt', 'w');
...
saveas(gcf, 'liu_exp_fit1/correlation_heatmap.png');
...
save('liu_exp_fit1/final_results.mat', 'final_results');

fileID = fopen('liu_exp_fit1/detailed_results.txt', 'w');
...
for i = 1:num_starts
    ...
    writematrix(loss_data, sprintf('liu_exp_fit1/loss_curve_run_%d.csv', i));
end
...
legend;
hold off;
saveas(gcf, 'liu_exp_fit1/loss_curves.png');
