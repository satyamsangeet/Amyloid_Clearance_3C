...
function plot_parameter_space(solutions, num_starts)
    ...
    saveas(gcf, 'liu_exp_fit_only_sigmacp/parameter_space.png');
    print('liu_exp_fit_only_sigmacp/parameter_space_highres', '-dpng', '-r300');

    % NEW CODE: Save parameter space data to CSV files
    for param_idx = 1:2
        % Create and open CSV file for this parameter
        param_name = param_names{param_idx};
        csv_filename = sprintf('liu_exp_fit_only_sigmacp/parameter_space_%s.csv', param_name);
...
if ~exist('liu_exp_fit_only_sigmacp', 'dir')
    mkdir('liu_exp_fit_only_sigmacp');
end
...
for i = 1:num_starts
    ...
    save(sprintf('liu_exp_fit_only_sigmacp/run_%d_results.mat', i), ...
        'run_results');
...
save('liu_exp_fit_only_sigmacp/final_results.mat', 'final_results');

% Create a text file to save results
fileID = fopen('liu_exp_fit_only_sigmacp/detailed_results.txt', 'w');
...
for i = 1:num_starts
    ...
    writematrix(loss_data, sprintf('liu_exp_fit_only_sigmacp/loss_curve_run_%d.csv', i));
end
...
legend;
hold off;
saveas(gcf, 'liu_exp_fit_only_sigmacp/loss_curves.png');
