            ...
            if length(csf_last_36hours_data) >= max(time_indices)
                ...
                if avg_C_model > 0
                    ...
                    if length(normalised_C_model) == length(exp_csf1)
                        total_error = sqrt(mean((normalised_C_model - exp_csf1).^2));
                        return;
                    end
                end
            end
...
function [best_params, best_fval] = sequential_multistart_optimization(num_starts, initial_guess, lb, ub, csf_conc_exp1)
    % Initialize storage
    best_params = [];
    best_fval = Inf;
    ...
    if ~exist('blattner_optimization_results_rmse', 'dir')
        mkdir('blattner_optimization_results_rmse');
    end
    ...
    for run = 1:num_starts
        ...
        initial_error = objective_function_fmincon(x0, csf_conc_exp1);
        ...
        this_run_objective = @(params) objective_function_wrapper(params, csf_conc_exp1, run);
        ...
        try
            ...
            % Save run results
            save(sprintf('blattner_optimization_results_rmse/run_%d_results.mat', run), 'params', 'fval', 'x0');
    ...
    if ~isempty(best_params)
        save('blattner_optimization_results_rmse/final_results.mat', 'best_params', 'best_fval');
    else
...
function total_error = objective_function_wrapper(params, exp_csf1, run_number)
    % Calculate error
    total_error = objective_function_fmincon(params, exp_csf1);
    ...
    if isempty(iteration_count)
        iteration_count = 1;
        filename = sprintf('blattner_optimization_results_rmse/run_%d_history.txt', run_number);
...
[best_params, best_fval] = sequential_multistart_optimization(num_starts, initial_guess, lb, ub, csf_conc_exp1);
