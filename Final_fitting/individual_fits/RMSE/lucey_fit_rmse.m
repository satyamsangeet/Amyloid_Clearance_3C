...
function [best_params, best_fval] = sequential_multistart_optimization(num_starts, initial_guess, lb, ub, csf_conc_exp1)
    ...
    if ~exist('lucey_optimization_results_rmse', 'dir')
        mkdir('lucey_optimization_results_rmse');
    end
    ...
    for run = 1:num_starts
        ...
        try
            ...
            % Save run results
            save(sprintf('lucey_optimization_results_rmse/run_%d_results.mat', run), 'params', 'fval', 'x0');
    ...
    if ~isempty(best_params)
        save('lucey_optimization_results_rmse/final_results.mat', 'best_params', 'best_fval');
    else
...
function total_error = objective_function_wrapper(params, exp_csf1, run_number)
    ...
    if isempty(iteration_count)
        iteration_count = 1;
        filename = sprintf('lucey_optimization_results_rmse/run_%d_history.txt', run_number);
...
experimental_csf1 = readtable('/home/satyam/Documents/combined_fitting_data/final_fitting/lucey2018_csf42.csv');
...
plot(time_exp/10, csf1_conc_exp, 'g--', 'LineWidth', 2.0, 'DisplayName', 'Lucey2018 - CSF');
