...
function total_error = objective_function_fmincon(params, exp_csf1, csf_std1)
    try
            ...
            if length(csf_last_36hours_data) >= max(time_indices)
                ...
                if avg_C_model > 0
                    ...
                    if length(normalised_C_model) == length(exp_csf1)
                        total_error = sqrt(sum(csf_weights1 .* (normalised_C_model - exp_csf1).^2) / sum(csf_weights1));
                        return;
                    end
                end
            end
...
function [best_params, best_fval] = sequential_multistart_optimization(num_starts, initial_guess, lb, ub, csf_conc_exp1, csf_std1)
    % Initialize storage
    best_params = [];
    best_fval = Inf;
    ...
    if ~exist('blattner_optimization_results_wmse', 'dir')
        mkdir('blattner_optimization_results_wmse');
    end
    ...
    for run = 1:num_starts
        ...
        initial_error = objective_function_fmincon(x0, csf_conc_exp1, csf_std1);
        ...
        this_run_objective = @(params) objective_function_wrapper(params, csf_conc_exp1, csf_std1, run);
        ...
        try
            ...
            % Save run results
            save(sprintf('blattner_optimization_results_wmse/run_%d_results.mat', run), 'params', 'fval', 'x0');
    ...
    if ~isempty(best_params)
        save('blattner_optimization_results_wmse/final_results.mat', 'best_params', 'best_fval');
    else
...
function total_error = objective_function_wrapper(params, exp_csf1, csf_std1, run_number)
    % Calculate error
    total_error = objective_function_fmincon(params, exp_csf1, csf_std1);
    ...
    if isempty(iteration_count)
        iteration_count = 1;
        filename = sprintf('blattner_optimization_results_wmse/run_%d_history.txt', run_number);
...
[best_params, best_fval] = sequential_multistart_optimization(num_starts, initial_guess, lb, ub, csf_conc_exp1, csf_std1);
...
fileID = fopen('/home/satyam/Documents/combined_fitting_data/final_fitting/combined_model_v1_output_new_values_blattner2020.csv', 'w');
...
saved_data = readmatrix('/home/satyam/Documents/combined_fitting_data/final_fitting/combined_model_v1_output_new_values_blattner2020.csv');
