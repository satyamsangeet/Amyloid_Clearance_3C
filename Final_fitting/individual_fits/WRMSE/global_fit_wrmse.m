...
% Main optimization loop with sequential runs
function [best_params, best_fval] = sequential_multistart_optimization(num_starts, initial_guess, lb, ub, csf_conc_exp1, csf_conc_exp2, csf_conc_exp3, plasma_conc_exp1,csf_std1, csf_std2, csf_std3, plasma_std1)
    ...
    if ~exist('global_optimization_results_wrmse', 'dir')
        mkdir('global_optimization_results_wrmse');
    end
    ...
    for run = 1:num_starts
        ...
        % Generate random initial point (except for first run which uses provided initial_guess)
        if run == 1
            x0 = initial_guess;
        else
        ...
        this_run_objective = @(params) objective_function_wrapper(params, csf_conc_exp1, csf_conc_exp2, csf_conc_exp3, plasma_conc_exp1, csf_std1, csf_std2, csf_std3, plasma_std1, run);
        ...
        % Save run summary
        save(sprintf('global_optimization_results_wrmse/run_%d_results.mat', run), 'params', 'fval', 'x0');
    end
    
    % Save overall results
    save('global_optimization_results_wrmse/final_results.mat', 'best_params', 'best_fval');
    
    % Create summary plot of all runs
    %plot_optimization_history(num_starts);
end

% Modified objective function wrapper for sequential runs
function [total_error] = objective_function_wrapper(params, exp_csf1, exp_csf2, exp_csf3, exp_plasma1, csf_std1, csf_std2, csf_std3, plasma_std1, run_number)
    % Calculate actual error
    total_error = objective_function_fmincon(params, exp_csf1, exp_csf2, exp_csf3, exp_plasma1, csf_std1, csf_std2, csf_std3, plasma_std1);
    ...
    if isempty(iteration_count)
        ...
        filename = sprintf('global_optimization_results_wrmse/run_%d_history.txt', run_number);
...
[best_params, best_fval] = sequential_multistart_optimization(num_starts, ...
    initial_guess, lb, ub, csf_conc_exp1, csf_conc_exp2, csf_conc_exp3, plasma_conc_exp1, csf_std1, csf_std2, csf_std3, plasma_std1);
