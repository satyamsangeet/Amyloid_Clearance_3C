...
function total_error = objective_function_fmincon(params, exp_csf1, csf_std1, exp_plasma1, plasma_std1)
    try
        ...
        if size(sol, 1) >= 237200
            ...
            if length(csf_last_36hours_data) >= max(time_indices)
                ...
                plasma_weights1 = 1./(plasma_std1.^2);
                ...
                if avg_C_model > 0
                    ...
                    if length(normalised_C_model) == length(exp_csf1)
                        C_error = sqrt(sum(csf_weights1 .* (normalised_C_model - exp_csf1).^2) / sum(csf_weights1));
                        P_error = sqrt(sum(plasma_weights1 .* (normalised_P_model - exp_plasma1).^2) / sum(plasma_weights1));
...
function [best_params, best_fval] = sequential_multistart_optimization(num_starts, initial_guess, lb, ub, csf_conc_exp1, csf_std1, plasma_conc_exp1, plasma_std1)
    % Initialize storage
    best_params = [];
    best_fval = Inf;
    ...
    if ~exist('liu_optimization_results_wmse', 'dir')
        mkdir('liu_optimization_results_wmse');
    end
    ...
    for run = 1:num_starts
        ...
        initial_error = objective_function_fmincon(x0, csf_conc_exp1, csf_std1, plasma_conc_exp1, plasma_std1);
        ...
        this_run_objective = @(params) objective_function_wrapper(params, csf_conc_exp1, csf_std1, plasma_conc_exp1, plasma_std1, run);
        ...
        try
            ...
            % Save run results
            save(sprintf('liu_optimization_results_wmse/run_%d_results.mat', run), 'params', 'fval', 'x0');
    ...
    if ~isempty(best_params)
        save('liu_optimization_results_wmse/final_results.mat', 'best_params', 'best_fval');
    else
...
function total_error = objective_function_wrapper(params, exp_csf1, csf_std1, exp_plasma1, plasma_std1, run_number)
    % Calculate error
    total_error = objective_function_fmincon(params, exp_csf1, csf_std1, exp_plasma1, plasma_std1);
    ...
    if isempty(iteration_count)
        iteration_count = 1;
        filename = sprintf('liu_optimization_results_wmse/run_%d_history.txt', run_number);
...
% Usage example:
num_starts = 20;
initial_guess = [0.005, 2, 5, 4];
lb = [0, 1, 1, 1];
ub = [0.0074, 10, 10, 10];

[best_params, best_fval] = sequential_multistart_optimization(num_starts, initial_guess, lb, ub, csf_conc_exp1, csf_std1, plasma_conc_exp1, plasma_std1);
