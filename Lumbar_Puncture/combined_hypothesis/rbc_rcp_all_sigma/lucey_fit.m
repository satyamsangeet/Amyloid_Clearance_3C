...
function [total_error] = objective_function_fmincon(params, exp_csf2, fitting_error0)
    ...
    fprintf('Parameters: r_bc=%.5f, r_cp=%.5f, s_A=%.5f, sigma_bc=%.5f, sigma_cp=%.5f, sigma_bp=%.5f, s_p=%.5f\n', ...
...
function plot_parameter_space(solutions, num_starts)
    ...
    param_names = {'r_bc', 'r_cp', 's_A', 'sigma_bc', 'sigma_cp', 'sigma_bp', 's_p'};
    ...
    % Save the figure
    saveas(gcf, 'lucey_fit1/parameter_space.png');
    print('lucey_fit1/parameter_space_highres', '-dpng', '-r300');
    
    % NEW CODE: Save parameter space data to CSV files
    for param_idx = 1:7
        % Create and open CSV file for this parameter
        param_name = param_names{param_idx};
        csv_filename = sprintf('lucey_fit1/parameter_space_%s.csv', param_name);
...
num_starts = 20;
lb = [0.062, 0, 0.485, 0, 0, 0, 0];
ub = [5, 0.0156, 0.99, 1.002, 4.087, 2.479, 4];
...
for i = 1:num_starts
    ...
    save(sprintf('lucey_fit1/run_%d_results.mat', i), ...
        'run_results');
...
fileID_corr = fopen('lucey_fit1/correlation_matrix.txt', 'w');
...
save('lucey_fit1/final_results.mat', 'final_results');
...
save('lucey_fit1/best_parameters.mat', 'best_params_struct');

% Also save just the raw best_params array if needed
save('lucey_fit1/best_params_array.mat', 'best_params');

% Create a text file to save results
fileID = fopen('lucey_fit1/detailed_results.txt', 'w');
...
for i = 1:num_starts
    ...
    fprintf(fileID, 'Parameters: r_bc=%.5f, r_cp=%.5f, s_A=%.5f, sigma_bc=%.5f, sigma_cp=%.5f, sigma_bp=%.5f, s_p=%.5f\n', ...
        solutions(i).params(1), solutions(i).params(2), solutions(i).params(3), solutions(i).params(4), solutions(i).params(5), solutions(i).params(6), solutions(i).params(7));
...
for i = 1:num_starts
    ...
    writematrix(loss_data, sprintf('lucey_fit1/loss_curve_run_%d.csv', i));
end
...
legend;
hold off;
saveas(gcf, 'lucey_fit1/loss_curves.png');
