...
function [total_error] = objective_function_fmincon(params, exp_csf2, fitting_error0)
    % Run simulation
...
function plot_parameter_space(solutions, num_starts)
    ...
    % Save the figure
    saveas(gcf, 'lucey_fit/parameter_space.png');
    print('lucey_fit/parameter_space_highres', '-dpng', '-r300');
    
    % NEW CODE: Save parameter space data to CSV files
    for param_idx = 1:6
        % Create and open CSV file for this parameter
        param_name = param_names{param_idx};
        csv_filename = sprintf('lucey_fit/parameter_space_%s.csv', param_name);
...
num_starts = 20;
lb = [0.062, 0, 0, 0, 0, 0];
ub = [5, 0.0156, 0.25, 1.002, 4.087, 2.479];
initial_guess = 0.5 * (lb + ub);

fitting_error0 = objective_function_fmincon(initial_guess, csf_conc_exp2);
...
problem = createOptimProblem('fmincon', ...
    'objective', @(params) objective_function_fmincon(params, csf_conc_exp2), ...
    'x0', initial_guess, ...
    'lb', lb, ...
    'ub', ub, ...
...
for i = 1:num_starts
    ...
    save(sprintf('lucey_fit/run_%d_results.mat', i), ...
        'run_results');
...
fileID_corr = fopen('lucey_fit/correlation_matrix.txt', 'w');
...
save('lucey_fit/final_results.mat', 'final_results');
...
save('lucey_fit/best_parameters.mat', 'best_params_struct');

% Also save just the raw best_params array if needed
save('lucey_fit/best_params_array.mat', 'best_params');

% Create a text file to save results
fileID = fopen('lucey_fit/detailed_results.txt', 'w');
...
for i = 1:num_starts
    ...
    writematrix(loss_data, sprintf('lucey_fit/loss_curve_run_%d.csv', i));
end
...
legend;
hold off;
saveas(gcf, 'lucey_fit/loss_curves.png');
