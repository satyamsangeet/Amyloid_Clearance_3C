...
writetable(model1_table, 'liu_model1_h3.csv');
writetable(model2_table, 'liu_model2_h3.csv');
...
extract_specific_timepoints('liu_model1_h3.csv', ...
                            fullfile('liu_model1_baseline.csv'), ...
                            start_time, end_time, step);

% Extract timepoints for Model 2
fprintf('Extracting timepoints for Model 2...\n');
extract_specific_timepoints('liu_model2_h3.csv', ...
                            fullfile('liu_model2_rbc_rcp_sbc_scp.csv'), ...
                            start_time, end_time, step);
...
g = (wrmse1_global1 + wrmse1_global3)/2;
e1 = (wrmse1_global2 + wrmse1_global4)/2;
final = ((e1 - g)/g)*100;
disp(round(final));

function results = calculate_combined_aic_aicc_bic( ...
    y_obs_comp2, y_obs_comp3, ...
...
%   results - struct containing AIC/AICc/BIC and diagnostics
    ...
    rss2 = sum(residuals_comp3.^2);

    % Number of observations
    n1 = length(y_obs_comp2);
    n2 = length(y_obs_comp3);
    n = length(y_obs_comp2) + length(y_obs_comp3);
...
model_file = 'liu_model2_rbc_rcp_sbc_scp.csv';
...
r_bc = ((r_bc_new - r_bc_old)/ r_bc_old)*100;
sigma_cp = ((sigma_cp_new - sigma_cp_old)/sigma_cp_old)*100;
sigma_p = ((sigma_p_new - sigma_p_old)/sigma_p_old)*100;
