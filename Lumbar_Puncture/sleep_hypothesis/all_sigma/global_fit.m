...
function [total_error] = objective_function_fmincon(params, exp_csf1, exp_csf2, exp_csf3, exp_plasma1, fitting_error0)
    ...
    errors_C2 = (csf_model2 - exp_csf2).^ 2;
    ...
    fitting_error = (nrmse_C1+nrmse_C2+nrmse_C3+nrmse_P1)/4;
    ...
    fprintf('Parameters: s_A=%.5f, s_bc=%.5f, s_bp=%.5f, s_vp=%.5f, s_p=%.5f\n', ...
            params(1), params(2), params(3), params(4), params(5));
    fprintf('Individual Study Errors: CSF1=%.5F, CSF2=%.5F, CSF3=%.5f, Plasma1=%.5f\n', ...
...
function plot_parameter_space(solutions, num_starts)
    ...
    params_matrix = zeros(num_starts, 5);
    ...
    param_names = {'s_A', 's_bc', 's_bp', 's_cp', 's_p'};
    ...
    % Save the figure
    saveas(gcf, 'global_fit1/parameter_space.png');
    print('global_fit1/parameter_space_highres', '-dpng', '-r300');
    
    % NEW CODE: Save parameter space data to CSV files
    for param_idx = 1:5
        % Create and open CSV file for this parameter
        param_name = param_names{param_idx};
        csv_filename = sprintf('global_fit1/parameter_space_%s.csv', param_name);
...
num_starts = 20;
lb = [0.772, 0, 0, 0, 0];
ub = [0.99, 1.131, 1.768, 6.1, 4.253];
initial_guess = 0.5 * (lb + ub);

fitting_error0 = objective_function_fmincon(initial_guess, csf_conc_exp1, csf_conc_exp2, csf_conc_exp3, plasma_conc_exp1);
...
problem = createOptimProblem('fmincon', ...
    'objective', @(params) objective_function_fmincon(params, csf_conc_exp1, csf_conc_exp2, csf_conc_exp3, plasma_conc_exp1), ...
    'x0', initial_guess, ...
    'lb', lb, ...
    'ub', ub, ...
...
for i = 1:num_starts
    ...
    save(sprintf('global_fit1/run_%d_results.mat', i), ...
        'run_results');
    
    % Log the results
    fprintf('\nRun %d:\n', i);
    fprintf('Parameters: s_A=%.5f, s_bc=%.5f, s_bp=%.5f, s_cp=%.5f, s_p=%.5f\n', ...
        solutions(i).params(1), solutions(i).params(2), solutions(i).params(3), solutions(i).params(4), solutions(i).params(5));
...
fileID_corr = fopen('global_fit1/correlation_matrix.txt', 'w');
...
save('global_fit1/final_results.mat', 'final_results');
...
save('global_fit1/best_parameters.mat', 'best_params_struct');

% Also save just the raw best_params array if needed
save('global_fit1/best_params_array.mat', 'best_params');

% Create a text file to save results
fileID = fopen('global_fit1/detailed_results.txt', 'w');
...
for i = 1:num_starts
    ...
    r_bc = 0.038;
    r_bp = 0.014;
    sigma_bc = solutions(i).params(2);
    sigma_bp = solutions(i).params(3);
    A = 16.203;
    sigma_A = solutions(i).params(1);
...
% Write best solution
fprintf(fileID, '\nBest Solution:\n');
fprintf(fileID, 'Parameters: s_A=%.5f, s_bc=%.5f, s_bp=%.5f, s_cp=%.5f, s_p=%.5f\n', ...
...
for i = 1:num_starts
    ...
    writematrix(loss_data, sprintf('global_fit1/loss_curve_run_%d.csv', i));
end
...
legend;
hold off;
saveas(gcf, 'global_fit1/loss_curves.png');
