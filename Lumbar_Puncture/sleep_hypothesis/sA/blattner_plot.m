...
writetable(model1_table, 'blattner_model1_h3.csv');
writetable(model2_table, 'blattner_model2_h3.csv');
...
extract_specific_timepoints('blattner_model1_h3.csv', ...
                            fullfile('blattner_model1_baseline.csv'), ...
                            start_time, end_time, step);

% Extract timepoints for Model 2
fprintf('Extracting timepoints for Model 2...\n');
extract_specific_timepoints('blattner_model2_h3.csv', ...
                            fullfile('blattner_model2_sA.csv'), ...
                            start_time, end_time, step);
...
% CORRECTED: Function to calculate NRMSE at experimental points only
function err = calculate_nrmse(model_data, exp_data)
    % model_data and exp_data should both be vectors of the same length (19 points)
    % corresponding to the experimental time points
    ...
    rmse = sqrt(mean(errors));  % Using mean instead of sum/length for clarity
    
    % Normalize by range of experimental data
    data_range = max(exp_data) - min(exp_data);
    
    % Calculate NRMSE
    err = rmse / data_range;
end

% Extract model predictions at experimental time points
csf_36hr_model_global1 = cdata_36hours1_global1(time_indices);
csf_36hr_model_global2 = cdata_36hours1_global2(time_indices);

% Calculate NRMSE for both models
wrmse1_global1 = calculate_nrmse(csf_36hr_model_global1, exp_csf1);
wrmse1_global2 = calculate_nrmse(csf_36hr_model_global2, exp_csf1);

% Print detailed comparison
fprintf('\n=== Model Comparison ===\n');
fprintf('Model 1 (Global Fit) NRMSE: %.6f\n', wrmse1_global1);
fprintf('Model 2 (Individual Fit) NRMSE: %.6f\n', wrmse1_global2);
fprintf('Difference: %.6f\n', wrmse1_global1 - wrmse1_global2);

% Calculate point-by-point errors for analysis
errors_model1 = abs(csf_36hr_model_global1 - exp_csf1);
errors_model2 = abs(csf_36hr_model_global2 - exp_csf1);
points_model1_better = sum(errors_model1 < errors_model2);
points_model2_better = sum(errors_model2 < errors_model1);

fprintf('\nPoint-by-point comparison:\n');
fprintf('Points where Model 1 is closer: %d\n', points_model1_better);
fprintf('Points where Model 2 is closer: %d\n', points_model2_better);
fprintf('Mean absolute error Model 1: %.2f pg/ml\n', mean(errors_model1));
fprintf('Mean absolute error Model 2: %.2f pg/ml\n', mean(errors_model2));
...
hold on;
plot(x, cdata_36hours1_global1, 'LineWidth', 2.0, 'Color', colormap_jet(1,:), 'DisplayName', sprintf('Model 1 (NRMSE: %.3f)', wrmse1_global1));
plot(x, cdata_36hours1_global2, 'LineWidth', 2.0, 'Color', colormap_jet(2,:), 'DisplayName', sprintf('Model 2 (NRMSE: %.3f)', wrmse1_global2));
...
xlim([0, 3600]);
title('CSF Amyloid Concentration: Global vs Individual Fit');
hold off;
...
fprintf('CSV files saved: blattner_model1_h3.csv, blattner_model2_h3.csv\n');
...
model_file = 'blattner_model2_sA.csv';
