...
function plot_parameter_space(solutions, num_starts)
    ...
    params_matrix = zeros(num_starts, 1);
...
% For each run
for run_idx = 1:num_starts  % Changed from 'i' to 'run_idx'
    fprintf(fileID, 'Run %d:\n', run_idx);
    fprintf(fileID, 'Parameters: s_A=%.5f\n', ...
        solutions(run_idx).params(1));  % Changed from 'i' to 'run_idx'
    
    % Calculate individual errors for this solution
    [t_run, sol_run] = euler(@(t,y) model(t,y,solutions(run_idx).params), [0, 24*100], [800,600,20], 0.01);
    
    % Extract last 36 hours and calculate errors
    ...
    sigma_bp = 1.768;
    A = 16.203;
    sigma_A = solutions(run_idx).params(1);  % Changed from 'i' to 'run_idx'
    ...
    for j = 1:length(time_analysis)  % Changed from 'i' to 'j'
