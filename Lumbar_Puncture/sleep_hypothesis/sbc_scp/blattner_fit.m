...
for i = 1:num_starts
    ...
    for j = 1:length(time_analysis)  % Changed from 'i' to 'j' to avoid conflict with outer loop
    ...
    fprintf(fileID, '  Fitting Error: %.6f\n', fitting_error);
    fprintf(fileID, '  Steady-State Penalty: %.6f\n', steady_state_penalty);
    fprintf(fileID, 'Total Error: %.6f\n\n', total_error);  % Added extra newline for spacing
end
