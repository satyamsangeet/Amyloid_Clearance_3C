...
extract_specific_timepoints('blattner_model2_h3.csv', ...
                            fullfile('blattner_model2_rbc.csv'), ...
                            start_time, end_time, step);
...
hold on;
plot(x, cdata_36hours1_global1, 'LineWidth', 2.0, 'Color', colormap_jet(1,:), 'DisplayName', sprintf('G (NRMSE: %.3f)', wrmse1_global1));
plot(x, cdata_36hours1_global2, 'LineWidth', 2.0, 'Color', colormap_jet(2,:), 'DisplayName', sprintf('GH (NRMSE: %.3f)', wrmse1_global2));
...
function results = calculate_combined_aic_aicc_bic( ...
    ...
    y_obs_comp2  = round(y_obs_comp2, 3);
    y_pred_comp2 = round(y_pred_comp2, 3);
...
model_file = 'blattner_model2_rbc.csv';
