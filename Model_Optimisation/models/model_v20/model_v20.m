...
function dydt_n = model(t, y, a21, a12_wake, a12_sleep, A_wake, A_sleep, k)
    sw_cycle = (mod(t,24) >= 8 && mod(t,24) < 24);
    dydt_n = zeros(2, 1);
    dydt_n(1) = A_wake * sw_cycle + A_sleep * (1 - sw_cycle) + a21 * y(2)- (a12_wake * sw_cycle + a12_sleep * (1 - sw_cycle))* y(1);
    dydt_n(2) = (a12_wake * sw_cycle + a12_sleep * (1 - sw_cycle))*y(1) - (a21 + k) * y(2);
end
...
initial_guess = [0.9, 0.6, 0.2, 50, 9.5, 10];
...
lossID = fopen('/home/satyam/Documents/MATLAB/Two_Compartment_Model/Optimisation/model_v20_loss.csv', 'w');
if lossID == -1
...
fileID = fopen('/home/satyam/Documents/MATLAB/Two_Compartment_Model/Optimisation/model_v20_output.csv', 'w');
...
for hour = 0:0.1:240
    try
        ...
        [t, sol] = ode45(@(t, y) model(t, y, a21_opt, a12_wake_opt, a12_sleep_opt, A_wake_opt, A_sleep_opt, k_opt), tspan, [exp_csf(1), exp_plasma(1)], opts);
