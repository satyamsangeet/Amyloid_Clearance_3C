...
function dydt_n = model(t, y, a12_wake, a13_wake, a23_wake, A_wake, A_sleep, a, b, k)
    sw_cycle = (mod(t,24) >= 8 && mod(t,24) < 24);
    dydt_n = zeros(3, 1);
    dydt_n(1) = A_wake*sw_cycle + (A_sleep*(1 - sw_cycle)) - (a13_wake*sw_cycle + (a13_wake*(1-sw_cycle))/b + a12_wake*sw_cycle + b*a12_wake*(1 - sw_cycle))*y(1);
    dydt_n(2) = (a12_wake*sw_cycle + b*a12_wake*(1-sw_cycle))*y(1) - (a23_wake*sw_cycle + a*a23_wake*(1-sw_cycle))*y(2);
    dydt_n(3) = (a23_wake*sw_cycle + a*a23_wake*(1-sw_cycle))*y(2) + (a13_wake*sw_cycle + (a13_wake*(1-sw_cycle))/b)*y(1) - k*y(3);
end
...
function total_error = objective_function(params, exp_csf, exp_plasma)
    ...
    [t, sol] = ode45(@(t, y) model(t, y, a12_wake, a13_wake, a23_wake, A_wake, A_sleep, a, b, k), [0, 24*10], [0, exp_csf(1), exp_plasma(1)], opts);
    
    csf_last_36hours_data = sol(20000:23600, 2);
    plasma_last_36hours_data = sol(20000:23600, 3);
...
initial_guess = [0.35, 0.22, 0.2, 10, 7.2, 0.8, 2, 0.2];

% Define bounds for parameters
lb = [0.1, 0.1, 0.1, 10, 7, 0, 0, 0.1];
ub = [0.4, 0.3, 0.3, 11, 9, 1, 5, 0.7];
...
lossID = fopen('/home/satyam/Documents/MATLAB/Two_Compartment_Model/Optimisation/model_v32_loss.csv', 'w');
if lossID == -1
...
fileID = fopen('/home/satyam/Documents/MATLAB/Two_Compartment_Model/Optimisation/model_v32_output.csv', 'w');
...
for hour = 0:0.1:240
    try
        ...
        [t, sol] = ode45(@(t, y) model(t, y, a12_wake_opt, a13_wake_opt, a23_wake_opt, A_wake_opt, A_sleep_opt, a_opt, b_opt, k_opt), tspan, [0, exp_csf(1), exp_plasma(1)], opts);
