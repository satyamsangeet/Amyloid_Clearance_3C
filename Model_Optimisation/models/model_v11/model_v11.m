...
function dydt_n = model(t, y, a12_wake, a12_sleep, A_wake, A_sleep, k)
    sw_cycle = (mod(t,24) >= 8 && mod(t,24) < 24);
    dydt_n = zeros(2, 1);
    dydt_n(1) = A_wake * sw_cycle + A_sleep * (1 - sw_cycle) - (a12_wake * sw_cycle + a12_sleep * (1 - sw_cycle))* y(1);
    dydt_n(2) = (a12_wake * sw_cycle + a12_sleep * (1 - sw_cycle))*y(1) - k*y(2);
end
...
initial_guess = [0.05, 0.125, 15, 6, 0.2];
...
fileID = fopen('/home/satyam/Documents/MATLAB/Two_Compartment_Model/Optimisation/model_v11_output.csv', 'w');
