...
function dydt_n = model(t, y, a12_wake, a13_wake, a23_wake, A_wake, A_sleep, a, k)
    ...
    dydt_n = zeros(3, 1);
    dydt_n(1) = A_wake * sw_cycle + (A_sleep * (1 - sw_cycle)) - (a13_wake * sw_cycle + a*a13_wake*(1-sw_cycle) + a12_wake*sw_cycle + 2.5*a12_wake*(1 - sw_cycle))*y(1);
    dydt_n(2) = (a12_wake*sw_cycle + 2.5*a12_wake*(1-sw_cycle))*y(1) - (a23_wake*sw_cycle + a*a23_wake*(1-sw_cycle))*y(2);
...
initial_guess = [0.5, 0.1, 0.2, 28, 12, 1, 1];

% Define bounds for parameters
lb = [0.5, 0, 0, 25, 10, 1, 1];
ub = [0.6, 0.1, 0.2, 30, 15, 1.5, 1.5];
...
lossID = fopen('/home/satyam/Documents/MATLAB/Two_Compartment_Model/Optimisation/model_v29_loss.csv', 'w');
if lossID == -1
...
fileID = fopen('/home/satyam/Documents/MATLAB/Two_Compartment_Model/Optimisation/model_v29_output.csv', 'w');
