...
function dydt_n = model(t, y, a12_wake, A_wake, a, k)
    sw_cycle = (mod(t,24) >= 8 && mod(t,24) < 24);
    dydt_n = zeros(2, 1);
    dydt_n(1) = A_wake * sw_cycle + (A_wake/a * (1 - sw_cycle)) - (a12_wake * sw_cycle + 2.5 * a12_wake * (1 - sw_cycle))* y(1);
    dydt_n(2) = (a12_wake * sw_cycle + 2.5 * a12_wake * (1 - sw_cycle))*y(1) - k * y(2);
end
...
function total_error = objective_function(params, exp_csf, exp_plasma)
    ...
    [t, sol] = ode45(@(t, y) model(t, y, a12_wake, A_wake, a, k), [0, 24*10], [exp_csf(1), exp_plasma(1)], opts);
...
lossID = fopen('/home/satyam/Documents/MATLAB/Two_Compartment_Model/Optimisation/model_v27_loss.csv', 'w');
if lossID == -1
...
fileID = fopen('/home/satyam/Documents/MATLAB/Two_Compartment_Model/Optimisation/model_v27_output.csv', 'w');
fprintf(fileID, 'Time,CSF,Plasma\n');
for hour = 0:0.1:240
    try
        opts = odeset('MaxStep',0.1);  % Define opts inside the loop
        tspan = [0, hour];  % Define tspan for each iteration
        [t, sol] = ode45(@(t, y) model(t, y, a12_wake_opt, A_wake_opt, a_opt, k_opt), tspan, [exp_csf(1), exp_plasma(1)], opts);
