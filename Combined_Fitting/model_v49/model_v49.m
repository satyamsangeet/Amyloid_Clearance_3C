...
function [total_error, is_valid] = objective_function(params, exp_csf, exp_plasma)
    global loss_values;
...
learning_rate = 0.01;
num_iterations = 15; % Set the correct number of iterations

for iter = 1:num_iterations
    [current_error, is_valid] = objective_function(params, exp_csf, exp_plasma);
    
    if ~is_valid
        % If constraints are violated, adjust parameters randomly
        params = lb + (ub - lb) .* rand(size(params));
        continue;
    end
    ...
    for i = 1:length(params)
        ...
        [new_error, is_valid_temp] = objective_function(params_temp, exp_csf, exp_plasma);
        if is_valid_temp
            grad(i) = (new_error - current_error) / learning_rate;
        else
            grad(i) = 0; % Don't move in directions that violate constraints
        end
    end
...
fileID = fopen('/home/satyam/Documents/MATLAB/Two_Compartment_Model/Optimisation/optimized_params_model_v49_clearance.csv', 'w');
fprintf(fileID, '%.15f,%.15f,%.15f,%.15f,%.15f,%.15f\n', optimized_params);
fclose(fileID);

% Save the loss values
lossID = fopen('/home/satyam/Documents/MATLAB/Two_Compartment_Model/Optimisation/model_v49_loss_clearance.csv', 'w');
if lossID == -1
...
fileID = fopen('/home/satyam/Documents/MATLAB/Two_Compartment_Model/Optimisation/model_v49_output_clearance.csv', 'w');
...
optimized_params = readmatrix('/home/satyam/Documents/MATLAB/Two_Compartment_Model/Optimisation/optimized_params_model_v49_clearance.csv');
...
saved_data = readmatrix('/home/satyam/Documents/MATLAB/Two_Compartment_Model/Optimisation/model_v49_output_clearance.csv');
